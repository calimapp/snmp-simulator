...
docker build . -t snmp-simulator:local
docker run -e CONFIG_FILEPATH=config.yaml -v $(pwd)/config.yaml:/config.yaml snmp-simulator:local
# Run from docker registry
docker run -e CONFIG_FILEPATH=config.yaml -v $(pwd)/config.yaml:/config.yaml ghcr.io/calimapp/snmp-simulator:latest
```
